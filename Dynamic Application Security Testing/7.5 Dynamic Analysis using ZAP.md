...
Include Ajax Spider
```sh
docker run --rm owasp/zap2docker-stable:2.10.0 zap-baseline.py -t https://prod-6jnpiomi.lab.practical-devsecops.training -j
```
Include a full scan, not just baseline
```sh
docker run --rm owasp/zap2docker-stable:2.10.0 zap-full-scan.py -t https://prod-6jnpiomi.lab.practical-devsecops.training -j 
```

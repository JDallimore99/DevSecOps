...
1. Six issues with confidence level Medium
2. Eleven issues with confidence level High
...
sast:
  stage: build
  script:
    - docker pull hysnsec/brakeman  # Download bandit docker container
    # Run docker container, please refer docker security course, if this doesn't make sense to you.
    - docker run --user $(id -u):$(id -g) -v $(pwd):/src --rm hysnsec/brakeman -r /src -f json -o /src/brakeman-output.json
  artifacts:
    paths: [brakeman-output.json]
    when: always
  allow_failure: true   #<--- allow the build to fail but don't mark it as such
```
Add Brakeman.ignore file to the GitLab file repository then add argument to the Gitlab-ci.yml file
```sh
image: docker:latest
